...
const ClassDetails = () => {
  ...
  useEffect(() => {
    fetch("/Classes.json")
      .then((res) => res.json())
      .then((data) => {
        setsClass(data.find((sData) => sData._id === id));
        // console.log(data);
      });
  }, [id]);
      ...
      <section className="bg-base-200 p-5 ">
        <div className="container flex flex-col mx-auto lg:flex-row">
          <div className="flex flex-col space-y-5  w-full p-4 lg:w-2/3 md:p-8 lg:p-10 lg:pt-5">
            ...
            <p className=" ">{sClass.short_description}</p>
            <div className="flex gap-2 items-center">
              {" "}
              <img
                alt=""
                className="w-10 h-10 rounded-full ring-[3px]  ring-blue-500 dark:bg-gray-500 "
                src="https://source.unsplash.com/40x40/?portrait?40"
              />
              <h3 className="text-xl font-semibold">
                Instructor: {sClass.name}
              </h3>
            </div>
            ...
            <h2 className="text-2xl font-semibold">
              ...
              <span className="text-3xl">{sClass.price}</span>
            </h2>
          </div>
        ...
        <div className="flex justify-between  gap-24">
          <button className="btn px-10 flex-1 text-lg font-medium rounded-3xl bg-primary-1 text-white hover:bg-primary-1 hover:bg-opacity-30 hover:border-primary-1 hover:border-2 hover:text-blue-700">
            PAY NOW
          </button>
